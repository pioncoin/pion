...
BOOST_AUTO_TEST_CASE(CreateNewBlock_validity)
{
    ...
    for (unsigned int i = 0; i < sizeof(blockinfo)/sizeof(*blockinfo); ++i)
    {
        ...
        pblock->nVersion = 4;
        pblock->nTime = chainActive.Tip()->GetMedianTimePast() + (i * chainparams.GetConsensus().nPowTargetSpacing) + 1;
        ...
        if (chainActive.Height() == 0) {
            txCoinbase.vout[0].nValue = 100000000000000LL;
        }
        else if (chainActive.Height() == 1) {
            txCoinbase.vout[0].nValue = 25000000000000LL;            
        }
        else if (chainActive.Height() > 1) {
            txCoinbase.vout[0].nValue = 2000000000LL;            
        }
