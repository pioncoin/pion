...
#if defined(NDEBUG)
# error "Pion Core cannot be compiled without assertions."
#endif
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    double reductionFactor = 0.84096;
    ...
    // yearly decline of production by ~15.9% per year, projected ~34M coins by year 2050+.
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue)
{
    ...
    int nMNPIPeriod = Params().GetConsensus().nMasternodePaymentsIncreasePeriod;
                                                                      ...
                                                                      // mainnet:
    if(nHeight > nMNPIBlock)                  ret += blockValue / 40; // 65700 - 32.5% 
    if(nHeight > nMNPIBlock + (nMNPIPeriod * 1)) ret += blockValue / 40; // 87600 - 35.0% 
    if(nHeight > nMNPIBlock + (nMNPIPeriod * 2)) ret += blockValue / 40; // 109500 - 37.5% 
    if(nHeight > nMNPIBlock + (nMNPIPeriod * 3)) ret += blockValue / 40; // 131400 - 40.0% 
    if(nHeight > nMNPIBlock + (nMNPIPeriod * 4)) ret += blockValue / 40; // 153300 - 42.5% 
    if(nHeight > nMNPIBlock + (nMNPIPeriod * 5)) ret += blockValue / 40; // 175200 - 45.0% 
    if(nHeight > nMNPIBlock + (nMNPIPeriod * 6)) ret += blockValue / 40; // 197100 - 47.5% 
    if(nHeight > nMNPIBlock + (nMNPIPeriod * 7)) ret += blockValue / 40; // 219000 - 50.0% 
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    // PION : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(PION): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(std::make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(PION): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // PION : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        LogPrintf("CheckBlock(PION): spork is off, skipping transaction locking checks\n");
    }
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Check proof of work. We are using DGWv2 from the start
