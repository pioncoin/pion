...
const char * const BITCOIN_CONF_FILENAME = "pion.conf";
const char * const BITCOIN_PID_FILENAME = "piond.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "pion" is a composite category enabling all Pion-related debug output
                if(ptrCategory->count(std::string("pion"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PionCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PionCore
    // Mac: ~/Library/Application Support/PionCore
    // Unix: ~/.pioncore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PionCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/PionCore";
#else
    // Unix
    return pathRet / ".pioncore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty pion.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override pion.conf
